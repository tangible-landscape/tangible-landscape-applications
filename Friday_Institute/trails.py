...
def calib_trails(real_elev, scanned_elev, env, **kwargs):
    env = get_environment(raster=real_elev)
    colors = ["0 green", "5 green", "5 yellow", "10 yellow", "10 red", "90 red"]
    gs.mapcalc("max_slope_trails = float(rand(0,91))", seed=1, env=env)
    gs.write_command(
        "r.colors", map="max_slope_trails", rules="-", stdin="\n".join(colors), env=env
    )
