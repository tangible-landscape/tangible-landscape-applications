...
def post_trails(real_elev, scanned_elev, filterResults, timeToFinish, logDir, env, **kwargs):
    ...
    logFile = os.path.join(logDir, 'log_{}_trails1.csv'.format(gisenv['LOCATION_NAME']))
    scoreFile = os.path.join(logDir, 'score_{}.csv'.format(gisenv['LOCATION_NAME']))
    slopes = gscript.list_grouped(type='raster', pattern="trails1_slope_dir_*_*_*")[gisenv['MAPSET']]
    lines = gscript.list_grouped(type='vector', pattern="trails1_line_*_*_*")[gisenv['MAPSET']]
